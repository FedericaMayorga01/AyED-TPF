...
void NetworkSimulator::initializeNetwork() {
    ...
    for (const auto& nodeConfig : config.nodes) {
        if (AddressUtils::isRouter(nodeConfig.node_address)) {
            ...
            std::cout << "Found router: " << nodeConfig.node_address 
                     << " (Router #" << AddressUtils::getRouterNumber(nodeConfig.node_address) << ")" << std::endl;
        } else if (AddressUtils::isTerminal(nodeConfig.node_address)) {
            terminalNodes.push_back(nodeConfig);
            std::cout << "Found terminal: " << nodeConfig.node_address 
                     << " (Router #" << AddressUtils::getRouterNumber(nodeConfig.node_address) 
                     << ", Terminal #" << AddressUtils::getTerminalNumber(nodeConfig.node_address) << ")" << std::endl;
        }
    }
    ...
    for (int address : routerAddresses) {
        ...
        Router router(address, initialRoutingTable);
        routers.push_back(router);
        
        std::cout << "Created router with address: " << address 
                 << " (Router #" << AddressUtils::getRouterNumber(address) << ")" << std::endl;
    }
    ...
    for (const auto& nodeConfig : config.nodes) {
        for (const auto& neighbor : nodeConfig.neighbors) {
            ...
            std::cout << "Created link: " << nodeConfig.node_address 
...
void NetworkSimulator::setupAdministrator() {
    ...
    for (const auto& link : links) {
        ...
        if (AddressUtils::isRouter(link.getSourceAddress()) && 
            AddressUtils::isRouter(link.getDestinationAddress())) {
